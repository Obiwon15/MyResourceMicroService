...
namespace Resourceedge.Appraisal.API.Services
{
    ...
    public class TeamService : ITeamRepository
    {
        ...
        public TeamService(IHttpClientFactory _httpClientFactory, ILogger<TeamService> _logger, IDbContext _dbContext, ITokenAccesor _tokenAccessor, IServiceFactory _serviceFactory)
        {
        ...
        public async Task<IEnumerable<OldEmployeeDto>> GetDistinctEmployeeIncludedinAppraisal(Participants participants)
        {
            var AllEmployeesIds = (await GetEmployeeIDs()).Select(x => x.ToString());

            if (participants == null)
            {
                    return await FetchEmployeesDetailsFromEmployeeService(AllEmployeesIds);
            }
            else if((participants.Include.Contains("Everyone") || participants.Include.Contains("everyone")))
            {
                if (participants.Exclude.Any())
                {
                    var IdsInBothLists = AllEmployeesIds.Intersect(participants.Exclude);
                    var employeesToGet = await FetchEmployeesDetailsFromEmployeeService(AllEmployeesIds.Except(IdsInBothLists));
            ...
            {
                return await FetchEmployeesDetailsFromEmployeeService(participants.Include);
            }
        }
