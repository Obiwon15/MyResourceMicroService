...
namespace Resourceedge.Appraisal.API.Services
{
    public class TokenAccessorService : ITokenAccesor
    {
        ...
        public TokenAccessorService(IHttpClientFactory _httpclient)
        {
            ...
            var aa = new Uri(httpclient.BaseAddress.OriginalString);
            DiscoveryDocument = httpclient.GetDiscoveryDocumentAsync(httpclient.BaseAddress.OriginalString).GetAwaiter().GetResult();
