...
namespace Resourceedge.Common.RabbitMq
{
    public class BusSubscriber : IBusSubscriber
    {
        ...
        public IBusSubscriber SubscribeCommand<TCommand>(string @namespace = null, string queueName = null,
            Func<TCommand, ResourceedgeException, IRejectedEvent> onError = null)
        ...
        public IBusSubscriber SubscribeEvent<TEvent>(string @namespace = null, string queueName = null,
           Func<TEvent, ResourceedgeException, IRejectedEvent> onError = null)
           where TEvent : IEvent
        {
        ...
        private async Task<Acknowledgement> TryHandleAsync<TMessage>(TMessage message,
            ...
            Func<Task> handle, Func<TMessage, ResourceedgeException, IRejectedEvent> onError = null)
        {
            var currentRetry = 0;
            ...
            {
                var scope = _tracer.BuildSpan("executing-handler")
                ...
                using (scope)
                {
                    ...
                    catch (Exception exception)
                    {
                        ...
                        if (exception is ResourceedgeException resourceedgeError && onError != null)
                        {
                            var rejectedEvent = onError(message, resourceedgeError);
