...
namespace Resourceedge.Common.Jaeger
{
    public class JaegerStagedMiddleware : StagedMiddleware
    {
        ...
        {
            var commandType = messageType.GetType();
            var spanBuilder = _tracer
                .BuildSpan($"processing-{messageType.Name}")
                .WithTag("message-type", $@"{(commandType.IsAssignableFrom(typeof(ICommand)) ? "command" : "event")}");
